...
We compare four factor-discovery criteria on a common train/test split:
    ...
    3. Empirical-Bayes shrinkage  : t-ratio shrunk toward zero, keep those
                                    whose shrunk |t| exceeds a q-quantile
                                    cutoff (a simple stand-in for the
                                    hierarchical-Bayesian factor selection of
...
def screen_empirical_bayes(data, ff, factor_model, q=0.05):
    """
    Simple empirical-Bayes shrinkage screen (INSPIRED BY, but NOT identical
    to, the hierarchical-Bayesian factor selection of Jensen, Kelly &
    Pedersen 2023). OLS alphas are shrunk toward zero under a conjugate
    Normal prior with an empirical-Bayes variance, and we keep factors whose
    shrunk |t|-ratio exceeds a q-quantile cutoff.

    We deliberately do NOT label this "JKP-Bayes": the full JKP model
    additionally shares information across 13 thematic clusters and across
    countries. This lightweight stand-in reproduces the qualitative effect
    of shrinkage on marginal alphas and is implemented so results are
    reproducible without the full JKP pipeline.
    """
